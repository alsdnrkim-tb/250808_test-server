...
public class AIServlet extends HttpServlet { // [1]
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        // git에 push할 때 이 dotenv가 push되지 않아서 배포에는 실행이 안됨
        Dotenv dotenv = Dotenv.configure()
                .ignoreIfMissing()
                .load(); // [7]
        // .env 파일이 없을 때 에러를 던지는 설정을 하는데 .configure, .ignoreIfMissing를 추가하여 없으면 무시하게 만듦
        ...
        String data = client.models.generateContent("gemini-2.0-flash",
                        "요즘 유행하는 MMORPG 알려주고, 결과만 50자 내로 출력해줘. 마크다운 같이 텍스트를 꾸미는 문법은 제외해.", null)
                .text(); // text를 불러와줌. [10]
